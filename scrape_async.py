...
async def crawl_website(domain, max_pages=100):
    ...
    raw_file = f"{domain.replace('.', '_')}_content_raw.json"
    with open(raw_file, "w", encoding="utf-8") as f:
        json.dump(results, f, ensure_ascii=False, indent=2)
    return raw_file

def clean_scraped_data(raw_path):
    with open(raw_path, "r", encoding="utf-8") as f:
        data = json.load(f)

    all_paragraphs = [p for page in data for p in page["paragraphs"]]
    ...
    out_path = "decom_cleaned.json"
    with open(out_path, "w", encoding="utf-8") as f:
        json.dump(cleaned_data, f, ensure_ascii=False, indent=2)
    return out_path

async def run_all(domain, max_pages):
    raw = await crawl_website(domain, max_pages)
    return clean_scraped_data(raw)
